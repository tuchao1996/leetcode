...
//  //      vector<int> ans(B.size(), -1);
//  //      vector<int> vec(A);
//		//auto itMaxVec = max_element(vec.begin(), vec.end());
//		//auto itMinB = min_element(B.begin(), B.end());
//		//if (*itMaxVec <= *itMinB) return vec;
//  //      for (int i = 0; i < B.size(); i++) {
//  //          sort(vec.begin(), vec.end());
//  //          int j = 0;
//  //          while (j < vec.size() && vec[j] <= B[i]) j++;
...
//		//while(left <= right) {
//		//	auto it = max_element(BB.begin(), BB.end());
//		//	int val = *it;
//		//	int index = distance(BB.begin(), it);
//		//	if (AA[right] > val) ans[index] = AA[right--];
//		//	else ans[index] = AA[left++];
//		//	BB[index] = -1;
//		//}
//		//return ans;
//
...
//			sell[i] = max(sell[i - 1], buy[i - 1] + prices[] - fee);
//		}
...
int main() {
	Solution s;
	vector<int> nums = { 10,100,11,9,100,10 };
	int ans = s.containsNearbyAlmostDuplicate(nums, 1, 2);
	cout << ans << endl;
